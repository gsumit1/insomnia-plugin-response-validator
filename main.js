...
module.exports.requestGroupActions = [
    {
        label: "Run All Requests-Parallel",
        action: async (context, data) => {
            ...
            await Promise.all(promiseList).then((responses) => {
                ...
                const html = `<ul>Error in request host, headers or reponse : ${map.get(reqNo[0])[1]}</ul>`;
    ...
    {
        label: "Run All Requests-Sequential",
        action: async (context, data) => {
            ...
            for (const request of requests) {
                ...
                try {
                    const entireResponse = await context.network.sendRequest(request)
                    try {
                        reqDetails = map.get(entireResponse.parentId);
                    } catch (error) { }
                    let reqMethod = reqDetails[0];
                    ...
                    let reqNo = error.message.match(/req_\w*/g)
                    console.log("Error in request", reqNo[0])
                    console.error(error.message);
                    results.push(`<tr><td>${map.get(reqNo[0])[0]} : ${map.get(reqNo[0])[1]}</td><td>Error in request, header or host</td></tr>`);
